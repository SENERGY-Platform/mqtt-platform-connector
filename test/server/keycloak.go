...
func Keycloak(pool *dockertest.Pool, ctx context.Context) (hostPort string, ipAddress string, err error) {
	log.Println("start keycloak")
	container, err := pool.Run("fgseitsrancher.wifa.intern.uni-leipzig.de:5000/keycloak", "dev", []string{
		"KEYCLOAK_USER=sepl",
	...
	err = pool.Retry(func() error {
		...
		resp, err := http.Post(
			"http://"+container.Container.NetworkSettings.IPAddress+":8080/auth/realms/master/protocol/openid-connect/token",
...
func ConfigKeycloak(url string, clientName string) (err error) {
	...
	req, err := http.NewRequest("POST", url+"/auth/admin/realms/master/partialImport", strings.NewReader(KEYCLOAK_CONFIG))
	if err != nil {
		return err
	}
