package server

import (
	library "github.com/SENERGY-Platform/platform-connector-lib"
	"github.com/SENERGY-Platform/platform-connector-lib/model"
	"github.com/SENERGY-Platform/platform-connector-lib/security"
	"time"
)

const AdminJwt = security.JwtToken("Bearer eyJhbGciOiJSUzI1NiIsInR5cCIgOiAiSldUIiwia2lkIiA6ICIzaUtabW9aUHpsMmRtQnBJdS1vSkY4ZVVUZHh4OUFIckVOcG5CcHM5SjYwIn0..eOwKV7vwRrWr8GlfCPFSq5WwR_p-_rSJURXCV1K7ClBY5jqKQkCsRL2V4YhkP1uS6ECeSxF7NNOLmElVLeFyAkvgSNOUkiuIWQpMTakNKynyRfH0SrdnPSTwK2V1s1i4VjoYdyZWXKNjeT2tUUX9eCyI5qOf_Dzcai5FhGCSUeKpV0ScUj5lKrn56aamlW9IdmbFJ4VwpQg2Y843Vc0TqpjK9n_uKwuRcQd9jkKHkbwWQ-wyJEbFWXHjQ6LnM84H0CQ2fgBqPPfpQDKjGSUNaCS-jtBcbsBAWQSICwol95BuOAqVFMucx56Wm-OyQOuoQ1jaLt2t-Uxtr-C9wKJWHQ")

func CreateDeviceType(conf library.Config, protocolSegmentName string) (typeId string, serviceTopic string, err error) {
	protocol := model.Protocol{}
	err = AdminJwt.PostJSON(conf.DeviceManagerUrl+"/protocols", model.Protocol{
		Name:             conf.Protocol,
		Handler:          conf.Protocol,
		ProtocolSegments: []model.ProtocolSegment{{Name: protocolSegmentName}},
	}, &protocol)
	if err != nil {
		return "", "", err
	}

	time.Sleep(10 * time.Second)

	//{"level": 42, "title": "event", "updateTime": 0}
	devicetype := model.DeviceType{}
	err = AdminJwt.PostJSON(conf.DeviceManagerUrl+"/device-types", model.DeviceType{
		Name: "foo",
		Services: []model.Service{
			{
				Name:        "sepl_get",
				LocalId:     "sepl_get",
				Description: "sepl_get",
				ProtocolId:  protocol.Id,
				Outputs: []model.Content{
					{
						ProtocolSegmentId: protocol.ProtocolSegments[0].Id,
						Serialization:     "json",
						ContentVariable: model.ContentVariable{
							Name: "metrics",
							Type: model.Structure,
							SubContentVariables: []model.ContentVariable{
								{
									Name: "updateTime",
									Type: model.Integer,
								},
								{
									Name: "level",
									Type: model.Integer,
								},
								{
									Name: "title",
									Type: model.String,
								},
							},
						},
					},
				},
			},
			{
				Name:        "exact",
				LocalId:     "exact",
				Description: "exact",
				ProtocolId:  protocol.Id,
				Inputs: []model.Content{
					{
						ProtocolSegmentId: protocol.ProtocolSegments[0].Id,
						Serialization:     "json",
						ContentVariable: model.ContentVariable{
							Name: "metrics",
							Type: model.Structure,
							SubContentVariables: []model.ContentVariable{
								{
									Name: "level",
									Type: model.Integer,
								},
							},
						},
					},
				},
			},
		},
	}, &devicetype)

	if err != nil {
		return "", "", err
	}
	time.Sleep(10 * time.Second)
	return devicetype.Id, model.ServiceIdToTopic(devicetype.Services[0].Id), nil
}
